...
bool is_valid_pos(uint8_t row, uint8_t col, uint8_t width, uint8_t height) {
...
/* Index lookup */
uint8_t get_index(uint8_t row, uint8_t col, uint8_t width, uint8_t height) {
  // printf("--butil geti-- row=%d col=%d w=%d h=%d\n",row,col,width,height);
  REQUIRES(is_valid_boardsize(width, height));
